function sm_scatter_ROI_z_scored_left_right_mean(ROI_index,ROI_names,SPM_path,atlas_path,data_path,dest)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is a function to plot mean of masked values of z-scored maps
...
%         vals_L = masked_map_L(masked_map_L~=0);
        vals_L = map_vols(ROI_L(:)>0);
        
        
        %calculate the median and mean value
        mean_L = mean(vals_L(~isnan(vals_L)));
        mean_Left = [mean_Left,mean_L]; %#ok<AGROW>
...
%         vals_R = masked_map_R(masked_map_R~=0);
        vals_R = map_vols(ROI_R(:)>0);
        
        %calculate the median and mean value
        mean_R = mean(vals_R);
        mean_Right = [mean_Right,mean_R]; %#ok<AGROW>
    ...
    Lreg_L = fitlm(age, mean_Left);
    Lreg_R = fitlm(age, mean_Right);
...
%     pvalue_L = Lreg_L.
    ...
    scatter(age, mean_Left,"diamond",'r');
    hold on;
    ...
    scatter(age, mean_Right,"o",'b');
      % Add regression line to scatter plot
    plot(age, coeffs_R.Estimate(1) + coeffs_R.Estimate(2)*age, "--b");
    
    title(['mean voxle values in ',char(plot_title), ' ROI']);
    xlabel('Age');
    ylabel('Mean');
    dim = [.2 .5 .3 .3];
%     text(15, 1.2, ['R_{L}^2 = ', num2str(R2_L)]);
%     text(15, 1.1, ['R_{R}^2 = ', num2str(R2_R)]);
